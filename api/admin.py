    ...
    def do_GET(self):
        try:
            ...
            if '/categories' in path:
                ...
                data = {'total_orders': total_orders, 'completed_orders': completed_orders, 'total_revenue': total_revenue, 'potential_revenue': potential_revenue, 'total_products': total_products, 'active_promocodes': active_promocodes}
            elif '/themes' in path:
                ...
                if is_admin:
                    ...
                    data = {'is_admin': True, 'is_active': admin_data.get('is_active', True), 'role': admin_data.get('role', 'manager'), 'first_name': admin_data.get('first_name', ''), 'username': admin_data.get('username', '')}
                else:
                    data = {'is_admin': False}
            self.send_response(200)
    ...
    def do_POST(self):
        try:
            ...
            if 'telegram_id' in data:
                ...
                category_data = {'name': data['name'], 'slug': data['slug'], 'icon': data.get('icon', 'fas fa-folder'), 'sort_order': data.get('sort_order', 0), 'is_active': data.get('is_active', True)}
